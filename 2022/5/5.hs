...
doInstruction (0,_,_)          = id
doInstruction (amount,from,to) = doInstruction (amount-1,from,to) . move from to

move :: StackNr -> StackNr -> [Stack] -> [Stack]
move f t s | f <= t = top ++ mid ++ (elem:tList) : end
    where
        (top,(elem:flist):tmp) = splitAt f     s
        (mid,tList:end)        = splitAt (t-f) (flist:tmp)
move f t s = top ++ (elem:tList) : mid ++ fList:end
    where
        (top,tList:tmp)        = splitAt t s
        (mid,(elem:fList):end) = splitAt (f-t-1) tmp
