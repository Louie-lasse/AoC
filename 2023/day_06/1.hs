...
parse :: [String] -> [Race]
parse [t,d] | (t',d') <- both (filter (not . null) . splitWhen (==' ') . dropWhile (not . isDigit)) (t,d)
            = zip (map read t') (map read d')

main = readFile "input.txt" >>= print . product . map alternatives . parse . lines
